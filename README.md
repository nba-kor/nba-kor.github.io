...
2. **내 프로필**을 한 번 등록한다: 게임 계정(닉네임·티어·캐릭터) 1~3줄 + 개인 마이크 O/X. 첫 줄 캐릭터가 대표다.
   **표시 이름**(로스터 · 알림 · 카톡 카드에 보이는 이름)은 처음에 디스코드 계정 이름으로 채워지고, 그 뒤로는 본인이 프로필에서 고친다 —
   디스코드 서버 별명이 계정 이름과 달라 서로 못 알아보는 일이 있어서다. 한 번 고치면 디스코드에서 이름을 바꿔도, 봇으로 프로필을 저장해도 그대로 남는다.
